...
async function parseAndRestoreFromURL() {
    const hash = window.location.hash;
    
    // Check for workspace URL: #/orders/123/workspace
    const workspaceMatch = hash.match(/#\/orders\/(\d+)\/workspace/);
    if (workspaceMatch) {
        const orderId = parseInt(workspaceMatch[1]);
        await openOrderWorkspace(orderId);
        return true;
    }
    
    // Check for other tab URLs
    if (hash === '#/orders') {
        switchOrderTab('orders');
        return true;
    }
    
    return false; // No URL to restore
}
...
async function initQuotesPage() {
    ...
    const restored = await parseAndRestoreFromURL();
    
    // If no URL state to restore, start on quotes tab
    if (!restored) {
        switchOrderTab('quotes');
    }
}

// Listen for browser back/forward navigation
window.addEventListener('popstate', async (event) => {
    // Try to restore from URL when user navigates with browser buttons
    await parseAndRestoreFromURL();
});
